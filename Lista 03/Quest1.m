...
Nh = 5;   % No. de nuronios na camada oculta
No = 10;   % No. de neuronios na camada de saida = numero de classes
...
for t=1:Ne,
    ...
    for tt=1:cP,   % Inicia LOOP de epocas de treinamento
        ...
        WW = WW + alfa*DDi*X';
        erroExemplo(tt,:) = Ek;
    end   % Fim do estocastico
    
    %valida��o ------------------------------------
    for tv=1:500,   % Inicia LOOP de epocas de treinamento
        % CAMADA OCULTA
        ...
        % CALCULO DO ERRO 
        Ek = T2(:,tv) - Ok;           % erro entre a saida desejada e a saida da rede
        erro_aux(tv) = sum(Ek.^2);
    end   % Fim do estocastico
    
    erro_val(t) = sum(erro_aux);
